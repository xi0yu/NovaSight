...
//! the Rust `CoordinateTransform` against the Python reference
//! implementation: the exporter writes expected values to
//! `rust/fixtures/phase2/*.jsonl`; this test parses the same files and
...
fn fixture_dir() -> PathBuf {
    // The exporter writes to `rust/fixtures/phase2/`, which lives two
    // directories up from this crate's `CARGO_MANIFEST_DIR`.
