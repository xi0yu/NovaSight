...
CONTROL_PARAM_SPECS: dict[str, ParamSpec] = {
    ...
    "control.universal_saturated.response_scale_x_px": _spec("control.universal_saturated.response_scale_x_px", "通用水平响应尺度", 160.0, 0.1, 4000.0, 0.1, "px", "X 误差进入明显非线性压缩的像素尺度。"),
    "control.universal_saturated.response_scale_y_px": _spec("control.universal_saturated.response_scale_y_px", "通用垂直响应尺度", 120.0, 0.1, 4000.0, 0.1, "px", "Y 误差进入明显非线性压缩的像素尺度。"),
    "control.universal_saturated.max_step_x_counts": _spec("control.universal_saturated.max_step_x_counts", "通用最大水平移动", 30.0, 0.1, 1000.0, 0.1, "counts", "通用饱和模式 X 输出渐近上限。"),
    "control.universal_saturated.max_step_y_counts": _spec("control.universal_saturated.max_step_y_counts", "通用最大垂直移动", 24.0, 0.1, 1000.0, 0.1, "counts", "通用饱和模式 Y 输出渐近上限。"),
    "control.shared.deadzone_x_px": _spec("control.shared.deadzone_x_px", "共享 X 像素死区", 0.0, 0.0, 10.0, 0.1, "px", "观测瞄点的 X 轴控制死区。"),
    "control.shared.deadzone_y_px": _spec("control.shared.deadzone_y_px", "共享 Y 像素死区", 0.0, 0.0, 10.0, 0.1, "px", "观测瞄点的 Y 轴控制死区。"),
