...
export function ActivityView({
  ...
  return (
    <section className="console-page activity-view" aria-labelledby="activity-view-title">
      <header className={items.length > 0 ? "activity-summary needs-attention" : "activity-summary is-clear"}>
        <span className="activity-summary-icon" aria-hidden="true">
          <NovaIcon name={items.length > 0 ? "triangle-alert" : "shield-check"} size={22} />
        </span>
        <div>
          <span>现在</span>
          <h2 id="activity-view-title">{items.length > 0 ? `${items.length} 件事需要注意` : "一切正常"}</h2>
          <p>{items.length > 0 ? "问题会保留在这里，直到恢复或由你清除本次会话的历史记录。" : "当前会话没有需要处理的故障。"}</p>
        </div>
        {items.length > 0 ? (
      ...
      <div className="activity-feed" aria-label="最近活动">
        {items.length > 0 ? items.map((item) => (
          <article className="activity-feed-item" key={item.key}>
