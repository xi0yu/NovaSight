...
不预设保留 DeepStream 或直接 TensorRT 最优。先测真实链路、设计缓冲区与调度，再通过同条件实验选型。现有代码和官方样例需要逐段审查；可运行、易复用或看起来更底层，都不是采用它们的充分理由。旧版计划中“先完整优化 DeepStream，最后才考虑直接 TensorRT”的顺序已撤回。
...
### P1：先设计数据通路，再做最小可比实验与选型

1. 用 P0 的实测约束设计目标通路：硬件视频解码 → NVMM → VIC/CUDA 图像预处理 → TensorRT → CUDA 解码/过滤/完整 NMS → 少量最终结果 → 原业务控制链。
2. 在实现前明确：谁持有采集帧；什么时候允许复用输入/输出缓冲区；各 stream 如何等待；如何有界地保留最新帧；结果如何绑定帧身份和时间戳；停止/切模型时如何禁止旧结果发布。逐项记录保留、融合、删除或重写的步骤及功能影响。
3. 比较两个调度候选：A 使用 DeepStream/nvinfer 衔接硬件缓冲和 GPU 后处理；B 由 NovaSight 持有硬件缓冲，直接管理 TensorRT context、CUDA stream/event 和结果交付。两者都需要完整 GPU 后处理，不能把 CPU 后处理基线与 GPU 候选的差异全部归因于框架。
4. 做单个已确认模型的隔离实验，固定 engine、输入、预处理语义、阈值、候选上限、精度和排队策略。尽可能使用同一组经过正确性检查的 CUDA 计算，改变调度/衔接部分。回放核对语义，真实采集验证硬件接入和帧龄；缺少真实输入时不作端到端选型结论。
5. 实验只回答缓冲接入、计算正确性、复制/同步和延迟的问题，不建立两套正式后端，也不复制完整产品 UI/业务模块。逐步补充 P0 已确认的重要负载，不能凭最小模型一次跑分确定全部模型适用。

P0 的有目标输入、帧龄和 RK3588 对照尚未齐备时，已确认的 CPU 后处理迁移要求仍有效：可继续梳理原算法语义、设计设备缓冲并验证共享 CUDA 计算。未齐备的测量阻止性能选型与产品验收，不应使 GPU 工作无限等待。

交付：两条候选的数据流/时序图、统一条件下的阶段和端到端对比、正确性结果，以及“选择哪条、为何更合适、还缺什么证据”的选型结论。现有 native TensorRT 封装主要用于模型接入诊断且会回传输出，不能直接当作实时零拷贝候选。

通过条件：获选方案满足关键计算硬件执行、模型/功能可承接和缓冲生命周期要求，在可比测试中具有可重复的速度优势。差异处于测量波动内时，不能声称某框架更快，应检查共同瓶颈并如实记录结果。只有选型证据成立后，才扩大生产接入范围。

### P2：按选型结果完成 GPU 计算与产品接入

1. 实现/采用经过审查的 CUDA 解码、过滤和完整 NMS。确认设备输出生命周期及同步，避免完整原始输出回传主机再送回 GPU；仅最终少量检测结果交给 CPU。对复用实现明确支持范围、阈值行为、排序/数值语义和可维护性。
2. 若选择 A，验证关闭 nvinfer CPU 解析/聚类、设备输出和自定义后处理的组合，最终对象元数据仍通过现有 bridge 交付。`nvdspostprocess` 只是接入候选；7.1 版本标为 Alpha，默认库不是自动 CUDA 化，必须验证实际执行与稳定性。
3. 若选择 B，直接接入硬件图像缓冲和 TensorRT，GPU 计算完成后形成拥有所有权的 `DetectionBatch`；保留 supervisor、模型管理、准星/预览及控制边界。根据数据流设计实现必要适配，不把已有诊断封装直接当产品主线。
