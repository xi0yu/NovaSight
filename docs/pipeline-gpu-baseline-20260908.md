...
> 历史实验记录：文中的“当前”指相应实验时点，不是现在的生产入口。当前代码与任务边界见 [GPU 图片契约](gpu-image-pipeline.md)，正式接入的历史收据见 [接入记录](gpu-product-integration-20260908.md)。保留原始数字及其限制，不追认新版本已通过硬件验收。

**当时状态：P0 部分完成，不能据此选择框架或宣称产品提速。** 三轮实机计时证明当时空检测场景下 CPU YOLO parser 约 0.1 ms；尚无证据证明 NMS 是主要瓶颈。当时配置确实将原始推理输出回传 CPU 后处理，不满足关键视觉计算全部硬件执行的目标。
...
## 补测：240 FPS 目标与真实吞吐（同日）

用户指出：固定 120 FPS 输入下约 9% 的延迟收益没有回答 RK3588 的
240 FPS 推理表现。本轮因此分别测持续模型推理、真实采集视觉链和纯采集，
未改模型、分辨率、阈值、功耗模式、时钟或正式运行配置。此前固定 120 FPS
结果不是 Jetson 的吞吐上限；也不能从 2 ms GPU 时间的倒数代替实测 FPS。

### 持续 TensorRT 推理

使用同一只读 engine，Batch 1、输入 `[1,3,256,256]`、单执行流、随机输入，
`trtexec --noDataTransfers --warmUp=2000 --duration=8 --infStreams=1`；普通与
`--useCudaGraph` 交替两轮。四次退出码均为 0：

| 模式 | 两轮实测吞吐（次/秒） | 两轮 GPU 平均计算时间（ms） | 两轮主机平均提交时间（ms） |
| --- | --- | --- | --- |
| 普通 TensorRT | 495.257 / 489.792 | 2.01578 / 2.03838 | 1.44857 / 1.44266 |
| CUDA Graph | 706.750 / 704.451 | 1.41222 / 1.41661 | 0.0125884 / 0.0119943 |

这些是工具实际完成数除以测量壁钟得到的吞吐，不是延迟倒数；不包含视频解码、
预处理、YOLO 后处理、结果拷贝或控制。Graph 的模型吞吐均值高约 43.3%，
不能称完整产品快 43.3%。与前文固定 120 FPS 的 Graph 延迟结果并不矛盾：
负载与衡量指标不同。现有 engine 的持续推理超过 240，尚不能据此宣称胜过
对方不同模型/输入/精度/核数或不同计时边界的 240 FPS。

### 输入协商缺陷与约 202 FPS 的实测供帧限制

诊断提交 `1a7da8c` 加入 240 FPS 参数后，精确 `240/1` 请求无帧；纯采集
GStreamer 也报 `not-negotiated (-4)`。让设备在 239–241 FPS 范围自行协商后，
实际公布的是 **`5000000/20833`（约 240.00384 FPS）**。诊断提交 `61d08ba`
改用该设备真实分数，采集成功。

这暴露生产源的一处同类问题：`v4l2.rs::enumerate_intervals` 调用
`rounded_fps` 丢弃分母，`deepstream/pipeline.rs` 为 MJPG 与未压缩输入均构造
`framerate={fps}/1`。本轮仅修复显式诊断 fixture，生产模式枚举到管线构造的
精确帧率传递尚未修复；不能将整数显示值直接视作可协商的 caps。

修正后，四轮 1080p240 请求均使用原 320px ROI / 256px engine 和 GPU 后处理，
预热 2 秒、测量 10 秒。所有检测结果均为空，物理控制不在该程序中：

| 模式 | 源出帧率（帧/秒） | 检测结果率（次/秒） | JPEG 解析后至结果 P95（ms） |
| --- | --- | --- | --- |
| 普通，第 1 轮 | 202.091 | 202.091 | 8.775 |
| Graph，第 1 轮 | 202.027 | 202.027 | 8.471 |
| 普通，第 2 轮 | 202.024 | 202.024 | 8.776 |
| Graph，第 2 轮 | 202.027 | 202.027 | 8.508 |

普通模式 appsink 消费至结果的 P95 为 3.871 / 3.861 ms，Graph 为
3.340 / 3.345 ms。Graph 在此负载缩短处理时间，但源供帧率和结果率没有增加。
两个 Graph 运行的 JPEG→结果关联各有 1 个未匹配帧；其余两轮为 0，四轮
均无 PTS 歧义或逆序。不能将结果率近似相同写成逐帧无损保证。

去掉解码、预处理和全部推理，仅 `v4l2src → fakesink`，两轮稳态供帧率为
**202.003 / 202.008 FPS**。进一步去掉 GStreamer，使用原生
`v4l2-ctl --stream-mmap=4 --stream-count=1200 --stream-to=/dev/null`，在驱动
报告 240.004 FPS 的配置下仍显示 **202.01–202.18 FPS**；首次速率报告同时
记录 42 个 dropped buffers。设备实际 USB 链路为 **5000 Mbit/s**。
因此当前约 202 FPS 的限制在输入采集链已经出现，不能归给 TensorRT、CUDA
NMS 或候选的串行等待。具体属于信号源、采集卡、驱动还是传输调度，仍未定位。

120→202 的结果率增长来自提高诊断请求帧率，**不是代码计算速度提升 68%**。
仍未证明 240 个有效新画面/秒、更未证明全功能识别/控制通过。
带不同 PTS 的采集帧不证明上游画面内容每帧更新。

### 对优化方向的修正

- CPU raw parser / NMS 必须迁移以满足硬件执行要求，但空检测 parser 约
  0.1 ms 不能解释吞吐差距。此前把 GPU 后处理落地当作主要性能进展并不充分。
- 当前单帧输入/输出槽、每帧等待及 EGL 注册仍是潜在开销；在更高输入负载
  或解耦的 GPU 预处理→推理→NMS 压测证明受限前，不宣称异步双缓冲必然提速。
- 下一步保持两个独立验收：解耦输入供帧后的 GPU 完整计算持续吞吐，以及
  真实采集到产品 `DetectionBatch` / 控制计划的功能、帧龄和有效更新率。
  修复生产精确帧率传递，并定位输入为何不足 240，不能用加深队列隐藏延迟。
- GPU 候选仍未接入生产生命周期、准星和控制。隔离计算成绩不能充当产品交付。

本地原始证据在被忽略的
`out/diagnostics/jetson-gpu-20260908/throughput-1788869016226920564/`：
四份 `*-ordinary.log` / `*-graph.log` 与逐次时间、`capture-*`、`source-only-*`、
`v4l2-direct.log`、`receipt.json`。最终检查采集设备释放、无产品进程，
engine 哈希保持 `0d034825230d8c01a5a451428082cfa01b6e0ad877e812d5c99dc57835187034`。
诊断增量编译、两项既有 CTest 和未知/重复参数拒绝检查通过；未切换产品。
