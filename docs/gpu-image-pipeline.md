...
最新实测：[2026-09-20 Jetson 收据](gpu-image-validation-20260920.md)。提交 `7822825` 已完成目标机构建、40 组 GPU 差分、内存检查、正式程序运行、故障注入及无 raw D2H 追踪；真实非空识别质量仍未验收。
...
边缘框使用 FP32 XYWH 传输，Rust 按 FP64 求右／下边界。GPU 最终打包使用向下舍入的剩余宽高限制框尺寸，避免 `90.04590606689453 + 165.9541015625 = 256.00000762939453` 这类舍入越界；不改变 NMS 选择，不放宽主机验证，也不在 CPU 修框。回归测试包含原始故障的横／纵边缘、两种布局、objectness 与 FP16/FP32。

运行故障在 Studio「异常信息」中同时展示说明与可展开的原始详情，保留错误码、子系统、daemon、epoch 和错误文本自带的帧标识。会话内沿用最近 20 条异常记录；状态刷新不重复计数，恢复后记录保留，页面刷新后清除。静默提示不再关闭异常记录；清空历史不会隐藏当前仍存在的运行故障。
...
[2026-09-08 接入收据](gpu-product-integration-20260908.md) 是更早的 GPU 执行证据；本次改动的设备验证见 [2026-09-20 收据](gpu-image-validation-20260920.md)。真实非空场景、全部模型／格式覆盖、长期稳定性仍需继续验证，不能从当前有限测试推导全部通过。
