...
模型目录浏览和强制重新校验都是只读操作。Studio 可在配置的模型根目录下新建文件夹；接口拒绝越界、已有名称和符号链接父目录，成功后重新读取目录树，不改变注册或部署。未登记且没有同目录 manifest 的 `.engine` 可以在明确确认后改名或移入已有文件夹；后端以不覆盖的原子重命名完成文件操作，再重新读取目录。若源路径在多个模型根目录中重名、目标已有文件、父目录为符号链接、或文件已登记/有 manifest，就拒绝而不动原文件。选择未登记 Engine 时，Registry 只保存原始文件路径和内容标识，不复制 Engine，也不创建哈希版本资产目录。已登记或活动模型的文件路径不能通过目录操作隐式改变。
